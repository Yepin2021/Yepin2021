(function(){
    ...
    var weiboMove = document.getElementById('wyy_a')
...
function btnChange(index,flag,speed){
    ...
    for(var n=0;n<nav_ul.length;n++){
        nav_ul[n].style.color = '#ffffff';
    }

    if(flag){
        if(index <= 0 ){
            indexs = 0;
            f_btn[0].style.background = "#3A82F5"; 
            //wrapBox.style.top = "0";
            nav_ul[0].style.color = '#397deb';
            divMove(wrapBox,0,20*speed);
        }
        else if(index >0 && index <=3){
            f_btn[index].style.background = "#3A82F5"; 
            //wrapBox.style.top = (-height * indexs) +"px";
            nav_ul[index].style.color = '#397deb';
